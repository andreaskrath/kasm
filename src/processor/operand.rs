use super::{constant::Word, error::InstructionError, register::Register};
...
impl Operand {
    pub fn parse(s: &str) -> Result<Operand, InstructionError> {
        match (s.parse::<Word>(), Register::parse(s)) {
            ...
            (Err(_), Err(_)) => Err(InstructionError::InvalidOperand(s))
        }
    }
}
